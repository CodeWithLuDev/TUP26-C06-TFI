...
const FOTOS_DESTACADOS = {
  azteca:  './azteca.jpg',
  metlife: './metlife.jpg',
  sofi:    './sofi.jpg',
};

function obtenerFoto(estadio) {
  return FOTOS_DESTACADOS[estadio.id] ?? estadio.imagen ?? '';
}
...
export function renderWikiEstadios() {
  ...
  destacados.forEach(({ id, badge }) => {
    ...
    card.innerHTML = `
      <img class="estadio-destacado__img" src="${obtenerFoto(e)}" alt="${e.nombre}" loading="lazy" onerror="this.style.display='none'" />
    ...
    lista.forEach(e => {
      ...
      card.innerHTML = `
        <div class="estadio-card__imagen-wrapper">
          <img class="estadio-card__imagen" src="${obtenerFoto(e)}" alt="${e.nombre}" loading="lazy" onerror="this.style.display='none'" />
        ...
        <div class="estadio-card__cuerpo">
          ...
          <div class="estadio-card__datos">
            <span class="estadio-dato"><span>👥</span><span class="estadio-dato__valor">${e.capacidad.toLocaleString('es-AR')}</span></span>
            <span class="estadio-dato"><span>📅</span><span class="estadio-dato__valor">${e.inaugurado}</span></span>
          </div>
        </div>
      `;
      const abrir = () => mostrarModalEstadio(e);
...
function mostrarModalEstadio(e) {
  ...
  contenido.innerHTML = `
    ...
    <div class="modal-estadio">
      <img class="modal-estadio__imagen" src="${obtenerFoto(e)}" alt="${e.nombre}" onerror="this.style.display='none'" />
      ...
      <div class="modal-equipo__stats">
        <div class="modal-stat"><span class="modal-stat__valor">${e.capacidad.toLocaleString('es-AR')}</span><span class="modal-stat__label">Capacidad</span></div>
        <div class="modal-stat"><span class="modal-stat__valor">${e.inaugurado}</span><span class="modal-stat__label">Inaugurado</span></div>
        <div class="modal-stat"><span class="modal-stat__valor">${e.superficie}</span><span class="modal-stat__label">Superficie</span></div>
      </div>
      <div class="modal-equipo__seccion">
        <h3 class="modal-equipo__subtitulo">Rol en el torneo</h3>
        <p>${e.partidos}</p>
      </div>
      <a href="${mapsUrl}" target="_blank" rel="noopener noreferrer" class="boton--maps">📍 Ver en Google Maps</a>
    </div>
  `;
